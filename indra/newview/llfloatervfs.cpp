...
void LLFloaterVFS::reloadEntry(entry file)
{
	...
	S32 file_size;
	apr_file_t* fp = ll_apr_file_open(file_name, LL_APR_RB, &file_size);
	if(fp)
	{
		LLVFile file(gVFS, file.mID, file.mType, LLVFile::WRITE);
...
void LLFloaterVFS::commitEdit()
{
	...
	if(!found) return;
	entry edited_file;
	edited_file.mName = childGetValue("name_edit");
	edited_file.mID = LLUUID(childGetValue("id_edit"));
...
void LLFloaterVFS::onClickAdd(void* user_data)
{
	...
	if(file_picker.getOpenFile(LLFilePicker::FFLOAD_ALL))
	{
		...
		S32 file_size;
		apr_file_t* fp = ll_apr_file_open(temp_filename, LL_APR_RB, &file_size);
		if(fp)
		{
