...
void LLAO::refresh()
{
	...
	for( ; sd_it != sd_end; sd_it++)
	{
		...
		if(mOverrides.find(sd_it->second) == mOverrides.end())
		{
			// ignore if override is null key...
			if(LLUUID(sd_it->second).notNull())
			{
				mOverrides[LLUUID(sd_it->first)] = sd_it->second;
			}
		}
	}
}
