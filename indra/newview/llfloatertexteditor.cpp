...
:	LLFloater()
{
	...
	mItem = item;
	LLUICtrlFactory::getInstance()->buildFloater(this, "floater_text_editor.xml");
}
...
void LLFloaterTextEditor::show(LLUUID item_id)
{
	...
	if(item)
	{
		...
		LLRect rect = gSavedSettings.getRect("FloaterTextEditorRect");
...
{
	LLTextEditor* editor = getChild<LLTextEditor>("text");
	mEditor = editor;
					...
					void* userdata)
{
	if(final)
	{
		...
		const char* src_data = (const char*)src->getData();
		char* buffer = new char[size + 1];
		memcpy(buffer, src_data, size);
		buffer[size - 1] = 0;

		std::string text(buffer);
		if(size > 0)
			LLStringUtil::replaceTabsWithSpaces(text, 4);
		delete[] buffer;

		floater->mEditor->setValue(text);
	...
	{
		src_vi->setBoostLevel(LLViewerImage::BOOST_UI);
	}
}
...
void LLFloaterTextEditor::onClickUpload(void* user_data)
{
	...
	std::string text = floater->mEditor->getText();
	...
	if (!file.write((const U8*)buffer, size))
	{
	...
	{
		upload_new_resource(transaction_id, item->getType(), item->getName(), item->getDescription(), 0, item->getType(), item->getInventoryType());
	}
