...
BOOL LLFloaterMessageLog::onClickCloseCircuit(void* user_data)
{
	...
	gViewerWindow->alertXml("GenericAlertYesNoCancel", args, onConfirmCloseCircuit, itemp);
	return TRUE;
}
// static
void LLFloaterMessageLog::onConfirmCloseCircuit(S32 option, void* user_data)
{
	LLNetListItem* itemp = (LLNetListItem*)user_data;
	LLCircuitData* cdp = itemp->mCircuitData;
	if(!cdp) return;
	...
	if(regionp)
	{
		...
		gViewerWindow->alertXml("GenericAlertYesNo", args, onConfirmRemoveRegion, regionp);
	}
}
// static
void LLFloaterMessageLog::onConfirmRemoveRegion(S32 option, void* user_data)
{
	LLViewerRegion* regionp = (LLViewerRegion*)user_data;
	if(option == 0) // yes
